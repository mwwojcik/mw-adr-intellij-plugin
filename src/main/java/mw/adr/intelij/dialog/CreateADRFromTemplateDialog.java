...
public class CreateADRFromTemplateDialog extends DialogWrapper {
  ...
  protected CreateADRFromTemplateDialog(@NotNull Project project) {
    super(project, true);
  ...
  public static CreateADRFromTemplateDialog.Builder createDialog(@NotNull final Project project) {
    if (Experiments.getInstance().isFeatureEnabled("show.create.new.element.in.popup")) {
      return new CreateADRFromTemplateDialog.NonBlockingPopupBuilderImpl(project);
    } else {
      final CreateADRFromTemplateDialog dialog = new CreateADRFromTemplateDialog(project);
      return new CreateADRFromTemplateDialog.BuilderImpl(dialog, project);
    }
  }
  ...
  public interface Builder {
    CreateADRFromTemplateDialog.Builder setTitle(String title);

    CreateADRFromTemplateDialog.Builder setValidator(InputValidator validator);

    CreateADRFromTemplateDialog.Builder addKind(
        @NotNull String kind, @Nullable Icon icon, @NotNull String templateName);

    @Nullable
    <T extends PsiElement> T show(
        @NotNull String errorTitle,
        @Nullable String selectedItem,
        @NotNull CreateADRFromTemplateDialog.FileCreator<T> creator);

    <T extends PsiElement> void show(
        @NotNull String errorTitle,
        @Nullable String selectedItem,
        @NotNull CreateADRFromTemplateDialog.FileCreator<T> creator,
  ...
  private static class BuilderImpl implements CreateADRFromTemplateDialog.Builder {
    private final CreateADRFromTemplateDialog myDialog;
    private final Project myProject;

    BuilderImpl(CreateADRFromTemplateDialog dialog, Project project) {
      myDialog = dialog;
      myProject = project;
    }

    @Override
    public CreateADRFromTemplateDialog.Builder setTitle(String title) {
      myDialog.setTitle(title);
      return this;
    }

    @Override
    public CreateADRFromTemplateDialog.Builder addKind(
        @NotNull String name, @Nullable Icon icon, @NotNull String templateName) {
    ...
    @Override
    public CreateADRFromTemplateDialog.Builder setValidator(InputValidator validator) {
      myDialog.myInputValidator = validator;
      return this;
    }

    @Override
    public <T extends PsiElement> T show(
        @NotNull String errorTitle,
        @Nullable String selectedTemplateName,
        @NotNull final CreateADRFromTemplateDialog.FileCreator<T> creator) {
      final Ref<SmartPsiElementPointer<T>> created = Ref.create(null);
      myDialog.getKindCombo().setSelectedName(selectedTemplateName);
      myDialog.myCreator =
          new ElementCreator(myProject, errorTitle) {

            @Override
            protected PsiElement[] create(@NotNull String newName) {
              T element =
                  creator.createFile(
                      myDialog.getEnteredName(), myDialog.getKindCombo().getSelectedName());
              if (element != null) {
                created.set(
    ...
    @Override
    public <T extends PsiElement> void show(
        @NotNull String errorTitle,
        @Nullable String selectedItem,
        @NotNull CreateADRFromTemplateDialog.FileCreator<T> creator,
        Consumer<? super T> elementConsumer) {
  ...
  private static class NonBlockingPopupBuilderImpl implements CreateADRFromTemplateDialog.Builder {
    ...
    public <T extends PsiElement> void show(
        @NotNull String errorTitle,
        @Nullable String selectedItem,
        @NotNull CreateADRFromTemplateDialog.FileCreator<T> fileCreator,
        Consumer<? super T> elementConsumer) {
      CreateWithTemplatesDialogPanel contentPanel =
          new CreateWithTemplatesDialogPanel(myTemplatesList, selectedItem);
            ...
            protected PsiElement[] create(@NotNull String newName) {
              T element =
                  fileCreator.createFile(
                      contentPanel.getEnteredName(), contentPanel.getSelectedTemplate());
      ...
      JBPopup popup =
          NewItemPopupUtil.createNewItemPopup(myTitle, contentPanel, contentPanel.getNameField());
      contentPanel.setApplyAction(
          e -> {
            ...
            if (isValid) {
              ...
              String errorMessage =
                  Optional.ofNullable(myInputValidator)
